...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Use a small threshold to avoid constant state updates
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 ${scrolled ? 'py-2' : 'py-3'}`}>
        ...
        >
            ...
            {navLinks.map(link => {
              ...
              return (
                <Link 
                  key={link.path} 
                  href={link.path} 
                  className={`px-4 py-2 text-[0.9rem] font-medium rounded-full transition-all duration-300 whitespace-nowrap font-sans
          ...
          {/* Desktop Search */}
          <form onSubmit={handleSearch} className="hidden md:flex items-center bg-black/20 p-[3px] rounded-full border border-white/15 transition-all min-w-[220px] shadow-inner">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {status === "loading" ? (
              ...
              <>
                <Link href="/chat" aria-label="Messages" className="relative w-[38px] h-[38px] rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors">
                ...
                <button onClick={handleLogout} aria-label="Logout" className="hidden md:flex w-[38px] h-[38px] rounded-full bg-[#ff3b30]/10 border border-[#ff3b30]/30 text-[#ff3b30] items-center justify-center hover:bg-[#ff3b30]/20 transition-colors cursor-pointer">
                  <FaSignOutAlt aria-hidden="true" size={14} />
                </button>
              </>
            ) : (
              <>
                <Link href="/login" className="hidden md:block px-4 py-2 text-[#e0e0e0] font-medium text-[0.9rem] hover:text-white transition-colors">Login</Link>
                <Link href="/signup" className="hidden md:block px-4 py-2 text-white font-medium text-[0.9rem] rounded-full bg-gradient-to-br from-[#667eea] to-[#764ba2] shadow-[0_4px_15px_rgba(102,126,234,0.4)] transition-transform hover:scale-105">Sign Up</Link>
              </>
            )}

            {/* Hamburger Button - Only visible on mobile via `md:hidden` */}
            <button aria-label={menuOpen ? "Close menu" : "Open menu"} onClick={() => setMenuOpen(!menuOpen)} className="md:hidden flex items-center justify-center bg-transparent border-none text-white text-2xl cursor-pointer p-1">
      ...
      <div className={`md:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[1000] transition-opacity duration-300 ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setMenuOpen(false)} />

      {/* Mobile Menu Slide-out Panel */}
      <div className={`md:hidden fixed top-0 right-0 w-full max-w-[320px] h-screen bg-[#0c0c10]/95 backdrop-blur-xl p-8 z-[1001] flex flex-col transition-transform duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex flex-col gap-2 overflow-y-auto pb-4">
          {navLinks.map(link => {
            if (link.adminOnly && session?.user?.role !== 'admin') return null;
            return (
              <Link key={link.path} href={link.path} onClick={() => setMenuOpen(false)} className={`p-4 rounded-2xl text-[1.1rem] font-medium flex items-center gap-4 transition-colors ${isActive(link.path) ? 'bg-[#667eea]/15 border border-[#667eea]/30 text-white' : 'text-white border border-transparent hover:bg-white/5'} ${link.adminOnly ? 'text-[#FFD700]' : ''}`}>
                {link.label}
              </Link>
            );
          })}
          ...
          {session ? (
            <>
              <Link href="/chat" onClick={() => setMenuOpen(false)} className="p-4 text-white rounded-2xl text-[1.1rem] font-medium flex items-center gap-4 hover:bg-white/5 transition-colors">
              ...
              <Link href="/profile" onClick={() => setMenuOpen(false)} className="p-4 text-white rounded-2xl text-[1.1rem] font-medium flex items-center gap-4 hover:bg-white/5 transition-colors">
              ...
              <button onClick={handleLogout} className="mt-auto p-4 rounded-2xl text-[1.1rem] font-medium flex items-center justify-center gap-2 text-[#ff3b30] bg-[#ff3b30]/5 hover:bg-[#ff3b30]/10 w-full transition-colors cursor-pointer">
                <FaSignOutAlt aria-hidden="true" /> Logout
              </button>
            </>
          ) : (
            <>
              <Link href="/login" onClick={() => setMenuOpen(false)} className="p-4 text-white rounded-2xl text-[1.1rem] font-medium flex items-center justify-center mb-2 bg-white/5 hover:bg-white/10 transition-colors">Login</Link>
              <Link href="/signup" onClick={() => setMenuOpen(false)} className="p-4 text-white rounded-2xl text-[1.1rem] font-medium flex items-center justify-center bg-gradient-to-br from-[#667eea] to-[#764ba2] hover:opacity-90 transition-opacity">Sign Up</Link>
            </>
          )}
        </div>
      </div>
